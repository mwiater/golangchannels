...
echo -e "${CYANBOLD}Building Go app for ARM 64:${RESET} ${GREENBOLD}GOOS=linux GOARCH=arm64 go build -o bin/golangchannels-arm64 .${RESET}"
ERROR=$(GOOS=linux GOARCH=arm64 go build -o bin/golangchannels-arm64 . 2>&1 1>/dev/null)
status=$?
if test $status -ne 0
then
