...
echo -e "${CYANBOLD}Building Go app for Windows 64:${RESET} ${GREENBOLD}GOOS=windows GOARCH=amd64 go build -o bin/golangchannels.exe .${RESET}"
ERROR=$(GOOS=windows GOARCH=amd64 go build -o bin/golangchannels.exe . 2>&1 1>/dev/null)
status=$?
if test $status -ne 0
then
